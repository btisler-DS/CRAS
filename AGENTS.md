...
Phase 5D-8 connects only the typed authorization result to the existing Dispatcher. Raw text and transcripts cannot enter that bridge; physical hardware is not selected. The Phase 4 simulator remains canonical. Until a later phase is explicitly authorized:
