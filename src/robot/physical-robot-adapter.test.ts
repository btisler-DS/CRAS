...
describe("PhysicalRobotAdapter", () => {
  it("is passive until execute and sends only an authenticated bounded envelope", () => {
    const calls: string[] = [];
    const transport: PhysicalRobotTransport = { request(request) { calls.push(request.body); return { status: 200, body: '{"status":"executed","final_position":"physical-demo-complete"}' }; } };
    ...
    expect(JSON.parse(sent.payload)).toMatchObject({ grant_id: "grant_1", evidence_record_id: "evidence_1", action: { kind: "MEDICATION_DELIVERY", destination: "Room 312" } });
